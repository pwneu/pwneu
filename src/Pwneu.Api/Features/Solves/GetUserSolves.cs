...
public static class GetUserSolves
{
    ...
    internal sealed class Handler(
        ...
        IRequestClient<MemberRequest> client)
        : IRequestHandler<Query, Result<PagedList<UserSolveResponse>>>
    {
        ...
        {
            var userResponse = await client.GetResponse<UserResponse, UserNotFoundResponse>(
                new MemberRequest { Id = request.Id }, cancellationToken);

            if (userResponse.Is(out Response<UserNotFoundResponse> _))
            ...
            return userSolves.Items.Count <= 0
                ? Result.Failure<PagedList<UserSolveResponse>>(NotFound)
                : userSolves;
        }
    }
