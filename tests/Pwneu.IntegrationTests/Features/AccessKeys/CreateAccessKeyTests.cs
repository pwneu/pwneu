...
public class CreateAccessKeyTests(IntegrationTestsWebAppFactory factory)
    : BaseIntegrationTest(factory)
{
    ...
    public async Task Handle_Should_NotCreateAccessKey_WhenCommandIsNotValid()
    {
        ...
        // Act
        var createAccessKeyResults = await Task.WhenAll(
            createAccessKeys.Select(invalidAccessKeys => Sender.Send(invalidAccessKeys)).ToList()
        );
    ...
    public async Task Handle_Should_CreateAccessKey_EvenIfDateIsNotUtc()
    {
        ...
        var accessKey = DbContext.AccessKeys.FirstOrDefault(c =>
            c.Id == createAccessKeyResult.Value
        );
